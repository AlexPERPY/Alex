...
FROM (SELECT DISTINCT(p.id)
      ...
      WHERE p.company_id IN (SELECT id
                       ...
                       WHERE id IN(SELECT company_id
                              ...
                              WHERE is_first_round=1 
                              AND is_last_round=1)
                              AND status = 'closed')) AS uni_id 
                              LEFT JOIN education AS e ON uni_id.id=e.person_id
GROUP BY uni_id.id, e.instituition
...
SELECT inv_2011.country_code,
       inv_2011.total_2011,
       inv_2012.total_2012,
       inv_2013.total_2013-- отобразите нужные поля
